...
import { loadConfig, saveConfig, nativeAbsolute } from './config.mjs';
import { buildContext } from './policy.mjs';
...
Usage: node <plugin>/src/cli.mjs <command> [options]
  ...
  doctor [--probe]      Check runtime/config/CLI; --probe checks API schema, not task data
...
function parse(argv) {
  ...
  const rules = {
    hook: {},
    doctor: { '--probe': 'boolean', '--codex': 'value' },
    configure: { '--project-root': 'repeat', '--add-project-root': 'repeat', '--enable': 'boolean', '--disable': 'boolean', '--codex': 'value' },
...
async function main() {
  ...
  if (options.command === 'hook') {
    const context = buildContext(await readEvent(), config);
  ...
  if (options.command === 'configure') {
    ...
    emit({ saved: saveConfig(config), automaticEnabled: config.enabled, projectRootCount: config.projectRoots.length });
    return;
  }
  ...
  if (options.command === 'doctor') {
    ...
    let launch, cliError;
    try { launch = resolveCodex({ executable }); } catch (error) { cliError = error.message; }
    ...
    const automaticReady = config.enabled && roots.some(root => root.accessible);
    const result = {
      node: process.versions.node, platform: process.platform, architecture: process.arch,
      configPath: filename, automaticEnabled: config.enabled, automaticScopeReady: automaticReady,
    ...
    emit(result);
    if (!launch || (api.checked && !api.supported)) process.exitCode = 1;
    return;
  }
