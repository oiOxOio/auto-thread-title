...
首条用户请求足以确定主题后，优先在开展业务工具调用之前完成标题整理，再继续用户的工作，不等待最终答复。主题不明确或命名工具不可用时跳过，不为命名增加模型回合。如果任务在改名完成前被中断，标题仍可能保留原名；插件不会自行恢复中断任务。钩子成功注入命名规则不等于标题已经写入。
...
CI 已配置 Ubuntu、Windows、macOS × Node.js 22/24 的六种组合，见 [工作流](../../.github/workflows/title-plugin-tests.yml)。验证状态以对应提交的 Actions 结果和实机记录为准，模拟平台测试通过不等于完成各平台桌面验收。实机验收需覆盖安装和信任、桌面环境下运行时发现、自动作用域内/外行为、首条请求主题明确后在业务工具调用前完成改名、只读清单，以及明确确认后的隔离任务改名。发生中断时应区分钩子已注入、命名工具已调用和标题已写入，不能仅凭任务已启动判断命名结果。
