...
- 自动命名策略在首条请求足以确定主题后，优先于业务工具调用完成当前任务改名；不拖到最终答复。验证主题不明、命名工具不可用时跳过，以及仅限当前任务、最多一次、没有额外模型任务的边界。
- 自动钩子的 `thread/read` 仅针对当前 ID 且使用 `includeTurns: false`；本地验证 ID、目录、创建时间和已有标题，不使用或注入正文、`preview`。验证正常路径只注入短规则并省去模型侧读取，以及合规标题不注入。
- 验证元数据接口不可用或 startup 瞬间任务暂不可读时回退完整核验指令，身份或目录不匹配时跳过。实机检查首次 startup 的任务可读性和实际标题写入，不能只用已持久化任务证明启动路径可用。
- 额度优化分别记录注入指令长度、模型侧工具往返和本机 RPC 开销；不把字符数变化换算成未经测量的额度百分比。允许同批工具调用中先改名再执行用户业务，不新增 MCP 服务或模型任务。
- 中断发生在命名完成之前时，标题可能保留原名；插件不自动恢复中断任务，不补触发旧任务，不把钩子注入成功当作实际改名成功。
...
- 每个插件使用独立的 `version`，正式发布使用 `MAJOR.MINOR.PATCH`，例如标题插件的 `0.2.0`。修复递增 PATCH，新增兼容功能递增 MINOR，不兼容变更递增 MAJOR；0.x 阶段的不兼容变更递增 MINOR 并在更新记录中明确迁移要求。
- 安装产物改变后发布新版本，避免沿用已有缓存；同步维护插件 `CHANGELOG.md`，说明用户可观察的变化和必要的升级步骤。正式版本不带时间戳缓存后缀。
- 本地开发迭代可选择 `$plugin-creator` 的版本缓存后缀更新工具，保留基础版本、只替换单个 `+codex.<标记>` 后缀；正式发布时切回新的语义化版本号。
